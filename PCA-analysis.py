...
RPC_data_type = ['IC', 'CURRENT_RATIO', 'RATE_RATIO']
RPC_types = ['RE2', 'RE4']
N_RPC = len(RPC_types)
sep = '_'
data_labels = ['Current density ratio', 'Gamma rate ratio']

env_vars = ['envP', 'envT', 'envRH', 'gasT_in', 'gasT_out', 'gasRH_in', 'gasRH_out', 'gasRH_ratio']
RPC_vars = [[],[]]
...
if useall:
    ...
    env_to_use = ['envP', 'gasT_in', 'gasT_out', 'gasRH_ratio']
    env_labels = ['Env. Pres.', 'Supply gas Temp.', 'Exhaust gas Temp.', 'Exh./Sup. R.H. ratio']
    ylabels = ['Atmospherical Pressure ($hPa$)',
               'Supply gas Temperature ($^{\circ}C$)',
               'Exhaust gas Temperature ($^{\circ}C$)',
               'Supply/Exhaust Relative Humidity ratio']
...
# density current and gamma rate values
rpcsets = [[],[],[]]
...
scree.savefig('Scree_plot_Irr-Ref-Ratio.png')
scree.savefig('Scree_plot_Irr-Ref-Ratio.pdf')
...
PC_plots = []
labels = [' Irr/Ref Current Density Ratio', ' Irr/Ref Gamma Rate Ratio']
filenames = ['Data_separation_vs_Current_Density_Ratio', 'Data_separation_vs_Gamma_Rate_Ratio']

for p in range(2):
    PC_plots.append(plt.figure(figsize = (10,5)))
    for rpc in range(N_RPC):
        position = 121 + rpc
...
if useall:
    ...
    filenames = ['Data_separation_vs_Pressure',
                 'Data_separation_vs_Gas-Temperature-IN',
                 'Data_separation_vs_Gas-Temperature-OUT',
                 'Data_separation_vs_Gas-Humidity-RATIO']
...
Var_PCs, Var_PCs_ax = plt.subplots(1, 2, figsize = (6,3), sharex='col', sharey='row')
...
for rpc in range(N_RPC):
    ...
    if col == 0:
        Var_PCs_ax[col].set_ylabel('Principal Component')
    Var_PCs_ax[col].set_xlabel('Signal Strength')
    ...
    for i_var, var in enumerate(variables):
        ...
        for d in range(N_PCs):
            ...
            y.append(d+1)
        major = y
        Var_PCs_ax[col].scatter(x, y, c=colors[i_var], label=var)
        if i_var < 2:
            data_matrix[rpc].append(x)
        else:
            env_matrix[rpc].append(x)

    Var_PCs_ax[col].set_xlim([-2,2])
    Var_PCs_ax[col].set_ylim([0.5,N_PCs*1.5+2])
    Var_PCs_ax[col].xaxis.set_major_locator(ticker.FixedLocator([-2,-1,0,1,2]))
    Var_PCs_ax[col].yaxis.set_major_locator(ticker.FixedLocator(major))
    Var_PCs_ax[col].grid()
    label = RPC_types[rpc]
    Var_PCs_ax[col].legend(loc='upper center', ncol=2, title=label, fontsize = 'xx-small')
Var_PCs.tight_layout()
Var_PCs.savefig('Irr-Ref-Ratio-variation-Scores.png')
Var_PCs.savefig('Irr-Ref-Ratio-variation-Scores.pdf')
